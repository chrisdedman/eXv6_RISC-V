/**
 * @file riscv.h
 * @brief RISC-V specific definitions and inline functions for accessing
 * control and status registers.
 *
 * This header file contains definitions and inline functions for accessing
 * various RISC-V control and status registers. It includes functions for
 * reading and writing machine and supervisor mode registers,
 * as well as macros for handling page tables and page table entries.
 *
 */

#ifndef __ASSEMBLER__

// Returns the Hart ID (core) of the current processor.
...
// Writes to Machine Exception Program Counter (mepc)
...
// Writes to Supervisor Exception Program Counter (sepc)
...
// Read and write tp (thread pointer) which holds this core's hartid (core number)
...
// Page size and offset definitions
#define PGSIZE 4096              // bytes per page
#define PGSHIFT 12               // bits of offset within a page

// Page rounding macros
#define PGROUNDUP(sz)  (((sz) + PGSIZE - 1) & ~(PGSIZE - 1)) // Round up to the nearest page
#define PGROUNDDOWN(a) (((a)) & ~(PGSIZE - 1))               // Round down to the nearest page
...
// Shift a physical address to the right place for a PTE conversion
...
// Virtual address indexing macros
#define PXMASK          0x1FF                                         // 9 bits for page table index
#define PXSHIFT(level)  (PGSHIFT + (9 * (level)))                     // Shift for level
#define PX(level, va)   ((((uint64)(va)) >> PXSHIFT(level)) & PXMASK) // Page index extraction

/* Maximum virtual address.
MAXVA is one bit less than the max allowed by Sv39 to avoid having 
to sign-extend virtual addresses with the high bit set. */
